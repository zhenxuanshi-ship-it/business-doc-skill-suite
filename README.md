...
```
用户：按照写作技能套件写一个智慧园区项目建议书，长文模式，分节写

AI（总入口）：我理解你要写智慧园区项目建议书，采用长文+分节写模式。
推荐流程：Discovery → Writer → Review → Integrity → Reviser

确认后：

1. Discovery阶段
AI：先帮你梳理项目brief，请提供项目基本信息
用户：好的，项目是XXX...
AI：Brief已生成，是否继续写项目建议书？
...
用户：确认大纲
AI：开始写第1章...（写完）第1章完成，继续第2章吗？

用户：继续
AI：开始写第2章...（写完）第2章完成，继续第3章吗？
...
```
用户：用模板写一个技术架构方案，我的模板是 公司技术方案模板.md
...
```
用户：写一个电商APP的PRD，要包含用户模块、订单模块、商品模块
...
- **business-doc-writer** 四级功能拆解规则优化
  - 新增智能工作量评估（总字数估算公式）
  - 新增分章节分文件写作策略（按字数分层）
  - 新增分文件目录结构示例
  - L3展开要求细化：描述100字+、输入具体字段、输出具体字段、业务规则3条+、验收条件2条+
  - 新增写作检查清单（6项检查）
  - 常见问题处理FAQ
