...
在开始写四级功能拆解之前，先评估整体工作量：

**评估标准**：
- L1模块数量：每个L1约 500-800 字
- L2子系统数量：每个L2约 300-500 字
- L3功能点数量：每个L3约 400-600 字（含5要素）
- L4功能细节：每个L4约 100-200 字

**总工作量估算公式**：
```
总字数 ≈ L1数量×600 + L2数量×400 + L3数量×500 + L4数量×150
```
...
|---|---|
| < 5000字 | 可单文件输出 |
| 5000-10000字 | 按L1分章节，每章一个文件 |
| 10000-20000字 | 按L2分章节，每个L2一个文件 |
| > 20000字 | 按L3分章节，每个L3一个文件 |

### 分章节分文件写作规则（关键！）

**为什么需要分文件写**：
- 一次性写太多容易偷懒、内容浮于表面
- 分文件写可以确保每个功能点都有实质性内容
- 便于用户审核每个部分
- 避免"写到最后没力气了"导致后面内容缩水
...
2. **逐章节写作**
   - 每写完一个L1/L2/L3，单独保存一个文件
   - 写完一章再写下一章，不要一次写多个

3. **检查信息密度**
   - 每个L3必须包含5要素（描述、输入、输出、业务规则、验收条件）
   - 每个要素必须有实质性内容，不能只是一句话
   - L3正文内容至少 200-400 字/个
...
```
sections/
├── 04-建设目标与内容.md          # 主文件（包含概述和汇总）
├── 04.1-L1大模型底座平台.md     # L1-1
├── 04.2-L2智能知识库平台.md     # L1-2
├── 04.3-L3智能对话平台.md       # L1-3
├── 04.3.1-警察智能助手.md       # L2-3.1
├── 04.3.2-罪犯教育问答.md       # L2-3.2
└── 功能清单.md                   # 汇总表
```
...
#### L3 展开写法（功能点）
必须包含：
1. **功能描述**：这个功能是做什么的（至少100字）
2. **输入**：需要什么数据/参数（列出具体字段）
3. **输出**：返回什么结果（列出具体字段）
4. **业务规则**：处理逻辑和约束（至少3条）
5. **验收条件**：怎么算完成（至少2条）
...
**功能描述**：支持注册新的能力源，包括能力源名称、能力源类型、连接信息、认证方式等。能力源注册是整个平台的入口，所有外部能力接入都需要先完成注册流程。注册过程需要验证连接信息的有效性，确保后续调用能够正常工作。

**输入**：
- 能力源名称（必填，字符串，最长50字符）
- 能力源类型（必填，枚举：API/数据库/文件）
- 连接地址（必填，URL格式）
- 认证方式（必填，枚举：无/Basic/OAuth2/APIKey）
- 用户名（条件必填，当认证方式非"无"时）
- 密码（条件必填，当认证方式非"无"时）
...
**业务规则**：
1. 能力源名称在同一平台内唯一，不能与已有名称重复
2. 连接信息需要加密存储，密码不允许明文保存
3. 支持连接测试，可以在注册过程中验证连接是否有效
4. 注册成功后自动分配默认权限组

**验收条件**：
1. 能够成功注册各类能力源（API/数据库/文件）
2. 能够测试能力源连接是否正常
3. 能够查询已注册的能力源列表
4. 能够修改能力源基本信息
```
...
- [ ] 业务规则是否至少3条？
- [ ] 验收条件是否至少2条？
- [ ] 整体信息密度是否足够（不是凑字数但也不是敷衍）？

**如果某个L3不满足以上条件，必须补充内容！**

### 常见问题处理

**Q: 某个功能实在没什么可写的怎么办？**
A: 这种情况说明功能拆分不够细，需要继续拆分成更小的功能点。

**Q: 写太长会不会太啰嗦？**
A: 关键是信息密度，不是字数。如果内容有实质价值，越多越好。如果是为了凑字数而写，即使短也是失败的。

**Q: 写到后面没力气了怎么办？**
A: 这就是为什么要分文件写。每写完一个L2/L3就保存文件，这样即使后面累了，至少前面的内容是完整的。
